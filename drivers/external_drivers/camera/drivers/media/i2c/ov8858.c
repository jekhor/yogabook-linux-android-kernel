...
#if 0
	static u32 last_et = 0;
	static u32 last_ag = 0;
#else
				 ...
				 int dig_gain, u16 *hts, u16 *vts)
{
	...
	dev_info(&client->dev, "bingo...%s, exposure = %d, gain = %d, dig_gain = %d\n",
				  ...
				  struct v4l2_private_int_data *priv)
{
	...
	if (!dev->otp_data) {
		...
		dev_err(&client->dev, "ov8858_g_priv_int_data \n");
...
static int __ov8858_init(struct v4l2_subdev *sd)
{
	...
	if(ret<0 )
		dev_err(&client->dev, "ov8858 init cmd error !!!%s\n", __func__);*/

	 for (i = 0; i < retry; i++) {
        ret = ov8858_write_reg_array(client, ov8858_BasicSettings);
        if (ret) {
            printk("bingo...%s(%d): ov8858_BasicSettings write failed!\n", __func__, i);
           msleep(50);
            vlv2_plat_set_clock_freq(0, 0); // 0: OSC_CAM0_CLK;
                                            // 0: 19.2MHZ;
            vlv2_plat_configure_clock(0, 1); // 0: OSC_CAM0_CLK;
                                            // 1: ON;
           msleep(50);
      } else {
           break;
...
static void __ov8858_print_timing(struct v4l2_subdev *sd)
{
	...
	dev_dbg(&client->dev, "HBlank: %d nS:\n",
		(1000 * (dev->pixels_per_line - width) / 2)
            / (dev->vt_pix_clk_freq_mhz / 1000000));
	dev_dbg(&client->dev, "VBlank: %d uS:\n",
		((dev->lines_per_frame - height) * dev->pixels_per_line / 2)
            / (dev->vt_pix_clk_freq_mhz / 1000000));
}

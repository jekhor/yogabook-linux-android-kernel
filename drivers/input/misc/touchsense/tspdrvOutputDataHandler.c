...
**
** Description: 
**	   Output data handler module for TSP 4000 Edition. 
**	   This module supports multiple actuators and one output sample
**	   per update.
**
...
#error "SPI_HEADER_SIZE expected to be 3"
	...
	while (i < count)
	{
		...
		** TSP 4000 devices supports only one output per update.
		*/
		memcpy(&(g_SamplesBuffer[pInputBuffer->nActuatorIndex]), &outputDataBuffer[i], (SPI_HEADER_SIZE + 1));
		g_SamplesBuffer[pInputBuffer->nActuatorIndex].nBufferSize = 1;
...
bool SendOutputData(void)
{
	...
	for (i = 0; i < NUM_ACTUATORS; i++) 
	{
		...
		if (pCurrentActuatorSample->nBufferSize)
		{
			...
			ImmVibeSPI_ForceOut_SetSamples(i, 8, 1, &pCurrentActuatorSample->dataBuffer);
